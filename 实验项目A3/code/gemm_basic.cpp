...
int verify(float *cpu_res, float *gpu_res, int length) {
  ...
  for (int i = 0; i < length; i++) {
    if (std::fabs(cpu_res[i] - gpu_res[i]) > 1e-3) { // 修正了fabs为std::fabs
      err++;
...
int gemm(const int M, const int N, const int K, const int block_size, const int iterations, sycl::queue &q) {
...
int main() {
  ...
  sycl::queue my_gpu_queue(sycl::gpu_selector_v, propList);
